...
addClickEvent("btn-save-update", () => {
    if (!db) {
        alert("Gagal: Database Firebase belum terhubung (db null)!");
        return;
    }
    const log = updateInput ? updateInput.value.trim() : "";
    if (log === "") {
        alert("Gagal: Kotak teks update masih kosong!");
        return;
    }
